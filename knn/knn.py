...
if __name__ == '__main__':
    ...
    for i in range(m):
        for j in range(28):
            for k in range(28):
                if train_images[i][j][k]<55.0:
                    trainingMat[i, 28*j+k] = 0
                else:
                    trainingMat[i, 28*j+k] = 1
    ...
    for i in range(mTest):
        ...
        for j in range(28):
            for k in range(28):
                if test_images[i][j][k]<55.0:
                    vectorUnderTest[28*j+k] = 0
                else:
                    vectorUnderTest[28*j+k] = 1 
                #vectorUnderTest[28*j+k] = test_images[i][j][k]  #第i幅测试图
 
        Result = classify(vectorUnderTest, trainingMat, train_labels, 5)
