...
if __name__ == '__main__':
    ...
    for n in range(300):       #用于比较离散度的影响
        ...
        female_sum2=0.0
        for i in range(20):
            sum1,male_sum1,female_sum1=test_bayes(n+2,[1,3,5,12])
            sum2+=sum1
            male_sum2+=male_sum1
            female_sum2+=female_sum1

        acc.append(sum2/20)
        male_acc.append(male_sum2/20)
        female_acc.append(female_sum2/20)

    x=np.arange(2,302) #.astype(dtype=np.str)               #用于比较离散度
    y=np.array(acc)
    ...
    plt.ylabel("Accuracy")
    plt.xlabel("N")
    plt.title("the ralationship of Accuracy and dispersion")
    plt.grid(True)
    plt.show()
